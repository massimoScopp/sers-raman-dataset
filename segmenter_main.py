...
def main():
    ...
    for (label_data, segmenter) in segmenters:
        ...
        for k in dataset_raman.keys():
            ...
            X_train, X_test, y_train, y_test = train_test_split(segments_and_lables[0], segments_and_lables[1],
                                                                test_size=testset_ratio, shuffle=True)

            segmented_dataset.update(
                {k: {'X_train': X_train, 'y_train': y_train, 'X_test': X_test, 'y_test': y_test,
