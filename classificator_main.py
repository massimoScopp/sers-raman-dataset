...
def main():
    ...
    for file in files:
        ...
        for k in pi_dict.keys():
            if prima :
                prima = False
                x_train =pi_dict[k]['X_train']
                y_train = pi_dict[k]['y_train']
                x_test = pi_dict[k]['X_test']
                y_test = pi_dict[k]['y_test']
            else :
                xx_train = pi_dict[k]['X_train']
                x_train = np.vstack([x_train, xx_train])
                yy_train = pi_dict[k]['y_train']
                y_train = np.concatenate([y_train, yy_train], axis =None)
