...
from tensorflow.keras.layers import Dense, Activation, Dropout, BatchNormalization, SeparableConv1D, LSTM, \
    Attention, GRU
from tensorflow.keras.regularizers import L1L2
from tensorflow.keras.models import Sequential
from tensorflow.keras.optimizers import SGD, Nadam, Adam, Adamax, Adagrad, RMSprop
...
def build_neural_model(s,n_layers, neurons, dropout):
    n_layers = n_layers
    n_classes = 2
    neurons = neurons
    ...
    model.add(Input(shape=(s)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    model.add(Dropout(dropout_rate))
    print("Add layer:[1] with " + str(neurons) + " neurons.")
    model.add(Dense(neurons))


    for i in range(2, n_layers + 1, 1):
        #print("Add layer:[" + str(i) + "] with " + str(p_neuron_map[i]) + " neurons.")
    ...
    #print("Add output layer with " + str(n_classes) + " neurons [" + p_last_activation + "].")
    ...
    return model
def custom_precision_m(y_true, y_pred):
    true_positives = K.sum(K.round(K.clip(y_true * y_pred, 0, 1)))
    predicted_positives = K.sum(K.round(K.clip(y_pred, 0, 1)))
    precision = true_positives / (predicted_positives + K.epsilon())
    return precision

def custom_recall_m(y_true, y_pred):
    true_positives = K.sum(K.round(K.clip(y_true * y_pred, 0, 1)))
    possible_positives = K.sum(K.round(K.clip(y_true, 0, 1)))
    recall = true_positives / (possible_positives + K.epsilon())
    return recall

def custom_f1_m(y_true, y_pred):
    precision = custom_precision_m(y_true, y_pred)
    recall = custom_recall_m(y_true, y_pred)
...
def main():
    ...
    import csv
    with open('misurazioni_'+tipoMisurazione+tipoCell+'.csv', mode='w', newline='') as csv_file:
    ...
    for s in size:
        for ov in overlap:
            ...
            clf = build_neural_model(s,n_layers=10, neurons= 256, dropout= 0.15)

            clf.compile(optimizer=SGD(nesterov=True, learning_rate= 0.04), loss="binary_crossentropy", metrics=["accuracy",
                                                                       "mae",
                                                                        custom_f1_m,
                                                                        custom_precision_m,
                                                                        custom_recall_m
            ...
            clf = clf.fit(x_train, y_train,batch_size= 8, epochs= 50) #era epochs =100
            ...
            # #fine metriche J48
            # # #metriche con cross validation
            # # from sklearn.model_selection import StratifiedKFold
            # # from sklearn.model_selection import cross_val_score
            # # kfold = StratifiedKFold(n_splits=10, random_state=1, shuffle=True)
            # # cv_resultsAcc = cross_val_score(clf, x_train, y_train, cv=kfold, scoring='accuracy')
            # # cv_resultsF1 = cross_val_score(clf, x_train, y_train, cv=kfold, scoring='f1_macro')
            # # print("CROSS VALIDATION Accuracy per J48:", cv_resultsAcc)
            # # print("CROSS VALIDATION F1 score per J48:", cv_resultsF1)
            # # #fine
            #
            #
            #
            ...
            # pp =cm_dir+'/cm_Deep_'+tipoCell+'_'+str(s)+'_'+str(ov)+'.png'
            ...
            # # plt.savefig(pp)
            # # #fine
            # with open('misurazioni_'+tipoMisurazione + tipoCell + '.csv', 'a', newline='') as csv_file:
            ...
            #     #writer.writerow([nome, 'RandomForest', rfscoreAcc, rfscorePrec, rfscoreF1, rfscoreRec, rfscoreRocAuc])
